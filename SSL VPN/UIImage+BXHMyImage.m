...
- (NSString *)scaleToByteSize:(unsigned long)size
{
    UIImage *scaleImage = [self resizedImageWithSize:CGSizeMake(500, 500)];
    float scale = 1.0;
...
- (UIImage *)imagescaledToByteSize:(unsigned long)size
{
    float scale = 1.0;
    double maxBytes = size;
    float cut = 0.1;
    NSData *pictureData = UIImageJPEGRepresentation(self,scale);
    while([pictureData length] > maxBytes)
    {
        if ((scale - cut) <= 0)
        {
        ...
        scale = scale - cut;
        pictureData = UIImageJPEGRepresentation(self,scale);
    }
    return self;
}
